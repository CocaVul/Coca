...
<a name="R4vZR"></a>
### 1) Clone this repo
`$ git clone https://github.com/CocaVul/Coca.git`
<a name="HSOG5"></a>
### 2) Install Prerequisites
<a name="QvMQH"></a>
### 3) run `detectors/ReVeal/train.py` for vulnerability detection
<a name="gr77a"></a>
### 4) run `vulexplainer/explainer.py` for vulnerability explanation
<a name="gr77a"></a>
## Structure
``` 
...
│   │   ├── pretrain.py               <- Encoder for initial node embedding.
│   │   └── train.py                  <- Detection model training.
│   ├── Devign   
│   ├── IVDetect   
│   └── ReVeal   
│
├── models                            <- Contrastive learning framework.
│   ├── self_supervised               <- Self-supervised contrastive learning model.
│   └── Supervised                    <- Supervised contrastive learning model.
│
├── Preprocess                        <- Preprocessing scripts.
│
└── vulexplainer                      <- Explanation scripts.
```
